...
class AttentionViewController: TableViewController<AttentionViewModel> {
    var dataSource:RxTableViewSectionedReloadDataSource<HomeAttentionSection>!
    ...
    override func makeUI() {
        super .makeUI()
        //创建一个重用的单元格
        tableView.register(AttentionViewTableViewCell.self, forCellReuseIdentifier: "AttentionViewTableViewCell")
    ...
    override func bindViewModel() {
        super.bindViewModel()
        let input = AttentionViewModel.Input()
        let output = viewModel.transform(input: input)
        //创建数据源
        dataSource = RxTableViewSectionedReloadDataSource
          <HomeAttentionSection>(configureCell: {
            [weak self] (dataSource, tv, indexPath, element)  in
            let cell: AttentionViewTableViewCell = self?.tableView.dequeueReusableCell(withIdentifier: "AttentionViewTableViewCell", for: indexPath) as! AttentionViewTableViewCell
            //print("cell复用")
